...
import { type } from 'os'
  ...
  //
   ...
   async function indexDriver(uris: string | Array<string>, isGZ: boolean, isTest: boolean, attributesArr: Array<string>) {
  ...
  // Returns a promise that ixIxx finishes indexing.
   async function parseLocalGff3(gff3LocalIn: string, isGZ: boolean, isTest: boolean, attributesArr: Array<string>){
...
/*
  ...
  function parseGff3UrlWithGz(urlIn: string, isTest: boolean, attributesArr: Array<string>) {
    ...
    let promise = new Promise((resolve, reject) => {
      if (newUrl.protocol === "https:") {
        httpsFR
        .get(urlIn,  async function (response) => {
...
*/
  ...
  function parseLocalGzip(file: ReadStream, isTest: boolean, attributesArr: Array<string>){
    const unzip = createGunzip()
    let gZipRead: ReadStream = file.pipe(unzip)
    return indexGff3(gZipRead, isTest, attributesArr)
  } 

  function tryit(gff3Stream: ReadStream, isTest: boolean, attributesArr: Array<string>) {
  ...
  function indexGff3(gff3In: ReadStream, isTest: boolean, attributesArr: Array<string>) {
  ...
  async function recurseFeatures(record: RecordData, gff3Stream: ReadStream, attributesArr: Array<string>) {
    let recordObj = {}
    ...
    let promise = new Promise((resolve, reject) => {
      ixProcess.on('close', (code) => {
        if (code == 0) {
          ...
          console.log(`Indexing done! Check ${ixFileName} and ${ixxFileName} files for output.`)
          return code;
        }
        else {
          reject('fail')
